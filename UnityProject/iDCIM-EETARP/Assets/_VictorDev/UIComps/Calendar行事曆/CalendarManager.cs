...
using static VictorDev.SystemConfig;
...
namespace VictorDev.Calendar
{
    ...
    {
        public enum EnumSelectStyle
        {
            單一日期,
            起迄日期
        }

        [Header(">>> 日期選取類型")] public EnumSelectStyle selectStyle = EnumSelectStyle.起迄日期;

        [Header(">>> [單一日期] 點選單一日期時發送")] public UnityEvent<DateTime> onSelectedDateEvent = new UnityEvent<DateTime>();
        ...
        [Header(">>> [起迄日期] 結束日期時發送")] public UnityEvent<DateTime> onSelectedEndDateEvent = new UnityEvent<DateTime>();
        ...
        [Header(">>> [起迄日期] 是否允許選擇同一天")] public bool isAllowSelectSameDay = false;

        [Header(">>> 選擇語言 (中文或英文)")] public EnumLanguage enumLang = EnumLanguage.en_US;

        [Header(">>> UI組件")] [SerializeField] DayItem dayPrefab; // 每一天的 DayItem 預置
        ...
        public void SetDateTimeRange(DateTime startDate, DateTime endDate)
        {
            ...
            GenerateCalendar();
            onSelectedStartDateStrEvent.Invoke(startDate.ToString(DateTimeHelper.FullDateFormat));
            onSelectedEndDateStrEvent.Invoke(endDate.ToString(DateTimeHelper.FullDateFormat));
        ...
        void GenerateCalendar()
        {
            ...
            {
                DateTime prevMonthDate = new DateTime(previousMonth.Year, previousMonth.Month,
                    daysInPreviousMonth - (startDayOfWeek - 1) + i);
        ...
        private void SelectSingleDayHandler(DayItem dayItem)
        {
            ...
            onSelectedDateStrEvent.Invoke(result.ToString(DateTimeHelper.FullDateFormat));
        ...
        private void SelectRangeDateHandler(DayItem dayItem)
        {
            if (startDate == null) // 如果尚未選擇起始日期
            {
                ...
                onSelectedStartDateStrEvent.Invoke(result.ToString(DateTimeHelper.FullDateFormat));
            }
            else if (endDate == null && (isAllowSelectSameDay || dayItem.date != startDate)) // 已選擇起始日期但尚未選擇結束日期
            {
                ...
                onSelectedEndDateStrEvent.Invoke(result.ToString(DateTimeHelper.FullDateFormat));
