...
namespace _VictorDEV.XChart
{
    public class LineChartHandler : MonoBehaviour
    {
        ...
        [ContextMenu("- 設定顯示總筆數")]
        private void SetDataShowAmount() => SetDataShowAmount(maxShowAmount);
        ...
        [Header("[資料項] - SerieIndex索引, float資料列表")] [SerializeField]
        private List<DictionaryVisualizer<int, List<int>>> seriaDatas = new();

        [Header("X軸顯示總筆數")] [SerializeField] private int maxShowAmount = 7;
